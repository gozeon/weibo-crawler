...
class Weibo(object):
    ...
    def parse_weibo(self, weibo_info):
        ...
        weibo['text_link'] = ','.join(self.get_text_link(text_body))
    ...
    def get_text_link(self, text_body):
        """获取网页链接的俩姐"""
        if '网页链接' in text_body:
            data_url = etree.HTML(text_body).xpath('//a/@data-url')
            href = etree.HTML(text_body).xpath('//a/@href')
            if len(data_url):
                return data_url
            else:
                return href
    ...
    def print_one_weibo(self, weibo):
        ...
        print(u'微博正文 url：%s' % weibo['text_link'])
